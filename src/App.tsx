  ...
  const handleLogin = async () => {
    if (validateForm()) {
      try {
        ...
        // loginError from useAuth will display a message if available
      }
    }
  };
...
function MainApp() {
  ...
  const [currentScreen, setCurrentScreen] = useState<'login' | 'register' | 'main' | 'chat'>('login');
  const [activeTab, setActiveTab] = useState<'community' | 'chats' | 'profile'>('community');
  const [screenParams, setScreenParams] = useState<any>({});

  useEffect(() => {
    if (isLoading) return; // Loading bitmeden bir şey yapma

    if (isAuthenticated && !needsEmailVerification) {
      // Giriş başarılı → doğrudan ana ekrana git, döngü olmasın
      setCurrentScreen('main');
      setActiveTab('community'); // Opsiyonel: her zaman community'den başlasın
    } else {
  ...
  // Giriş yapılmamışsa sadece auth ekranları
